...
void readMTX(
    ...
    for (mwIndex l = 0; l < m[0]; l++) {
        ...
        if (issymmetric)
            if (row_coo[k] == col_coo[k])
                m_mat -= 2; // we do not keep self-loop, remove edges
            else {
                row_coo[k + 1] = col_coo[k];
                col_coo[k + 1] = row_coo[k];
                k += 2;
            } // put symmetric edge
        else if (row_coo[k] == col_coo[k])
            m_mat -= 1; // we do not keep self-loop, remove edge
        else
            k++;
    }
...
std::ostream &output(std::ostream &outfile, double *arr, int rows, int cols) {
    ...
    for (int i = 0; i < rows; i++) {
        ...
        outfile << arr[i * cols + cols - 1] << std::endl;
    }
    return outfile;
}
